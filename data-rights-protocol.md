# [Data Rights Protocol](https://github.com/consumer-reports-digital-lab/data-rights-protocol) v.0.7-draft
...
### 2.02 `POST /data-right-request/` ("Data Rights Exercise" endpoint)
...
This request SHALL contain an Bearer Token header containing the key for this AA-CB pairwise relationship in it in the form `Authorization: Bearer <token>`. This token is generated by calling `POST /key-exchange` in section 2.06.

The Privacy Infrastructure Provider SHALL validate the message is signed according to the guidance in section 3.07

#### 2.02.1 `POST /data-rights-request` Response

Responses to this request MUST adhere to the [Exercise Status Schema](#303-schema-status-of-a-data-subject-exercise-request).

### 2.03 `GET /data-rights-request/{request_id}` ("Data Rights Status" endpoint)
...
#### 2.03.1 `GET /data-rights-request/{request_id}` Response
...
### 2.05 `DELETE /data-rights-request/{request_id}` ("Data Rights Revoke" endpoint)
...
#### 2.05.1 `DELETE /data-rights-request/{request_id}` response
...
### 2.07 `GET /agent/{agent-id}` ("Agent Information" endpoint)
...
#### 2.07.1 `GET /agent/{agent-id}` Response
