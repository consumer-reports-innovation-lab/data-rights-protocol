...
This request SHALL contain an Bearer Token header containing the key for this AA-CB pairwise relationship in it in the form `Authorization: Bearer <token>`. This token is generated by calling `POST /agent/{id}/register` in section 2.06.
...
### 2.03 `GET /v1/data-rights-request/{request_id}` ("Data Rights Status" endpoint)
...
#### 2.03.1 `GET /v1/data-rights-request/{request_id}` Response
...
#### 2.05.1 `DELETE /v1/data-rights-request/{request_id}` response
...
### 2.06 `POST /v1/agent/{agent-id}/register` ("Pair-wise Key Setup" endpoint)
...
#### 2.06.1 `POST /v1/agent/{agent-id}/register` Response
...
### 2.07 `GET /v1/agent/{agent-id}` ("Agent Information" endpoint)
